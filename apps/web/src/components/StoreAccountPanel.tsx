...
export default function StoreAccountPanel({ branchId, branchName }: { branchId: string; branchName: string }) {
  ...
  async function createAccount() {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/create-employee`, {
        ...
        body: JSON.stringify({
          email,
          full_name: `${branchName} Ekranı`,
          branch_id: branchId,
          role_code: "store_display",
        }),
      });
  ...
  return (
    <div>
      <label style={labelStyle}>
        E-posta (bu ekranda giriş için kullanılacak)
        <input
          type="email"
          value={email}
          onChange={(e) => setEmail(e.target.value)}
          placeholder="magaza-ekrani@sirketiniz.com"
          style={inputStyle}
        />
      </label>
      <button onClick={createAccount} disabled={creating || !email.trim()} style={{ ...buttonStyle, opacity: creating ? 0.6 : 1 }}>
        <Store size={14} strokeWidth={2} />
        {creating ? "Oluşturuluyor..." : "Mağaza Hesabı Oluştur"}
      </button>
      ...
      {result && (
        <div style={resultBoxStyle}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
            ...
            <strong style={{ fontSize: 13 }}>Hesap oluşturuldu</strong>
          </div>
          ...
          <p style={{ fontSize: 11.5, color: "var(--text-secondary)", marginTop: 8 }}>
            Bu bilgileri not edin — fiziksel ekranda bu bilgilerle giriş yapın.
          </p>
        </div>
      )}
    </div>
